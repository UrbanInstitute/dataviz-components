...
Note: The tooltip is rendered using Svelte 5's snippet syntax. Pass a snippet prop to `SVGMap` instead of using a named slot:

```svelte
<SVGMap features={county_air_quality.features}>
...
Now you can pass a `featureHighlight` object that sets the highlighted feature based on the `clevelandHighlight` variable. We can also update the value of this variable in the `onclick` callback, allowing the user to set a highlighted feature when clicking. Combining this together would like like the following example:

<Canvas of={Stories.FeatureHighlight} sourceState="shown" />

## Event handling (Step 2)

- Map-level events: `SVGMap` uses lowercase callback props (`onclick`, `onmousemove`, `onmouseout`, and `onbgclick`).
- Layer events: In Step 2, layers still dispatch Svelte 4 custom events. Use `on:` directives on layers (migration to callback props happens in Step 3).

Layer events (current):
...
Note: Background clicks trigger `onbgclick` only - they do not trigger `onclick`. This allows you to handle background and feature clicks separately.

Coming in Step 3: Layers will adopt lowercase callback props (e.g., `onclick`, `onmousemove`). The examples above will be updated accordingly when that migration lands.
...
## Advanced: Internal context changes

For developers extending or customizing map layers, `SVGMap` now uses a rune-based context system internally. The context is created with `createSVGMapContext()` and consumed by layers using `useSVGMapContext()`. This replaces the previous Svelte store-based context.

The context provides:

- `projection` - The D3 projection function (updated via `fitSize`)
- `transform` - The D3 zoom transform state
- `features` - The feature collection used for fitting
- `stickyHighlight` - Currently highlighted feature props (or `null`)
- `handleLayerMousemove(tooltipProps)` - Method for layers to call on mousemove
- `handleLayerClick(tooltipProps)` - Method for layers to call on click

This is primarily an internal implementation detail. Layer components (`SVGPolygonLayer`, `SVGPointLayer`, `SVGLabelLayer`) will be migrated to use this new context in a future update. For now, both the new rune-based context and the legacy context are maintained for compatibility.
