...
`SVGMap` and its layers use lowercase callback props for event handling. Both map-level and layer-level events can be handled using `onclick`, `onmousemove`, and `onmouseout` props.

Layer and map callbacks fire from pointer events internally (pointerdown/move/out), exposed as onclick, onmousemove, onmouseout props for API compatibility.

Layer events use callback props:
...
## Advanced: Custom layers with rune-based context

For developers creating custom map layers, `SVGMap` uses a rune-based context system via `createSVGMapContext()` and `useSVGMapContext()`. The map component calls `createSVGMapContext()` internally, so layer components (including external ones) only need to call `useSVGMapContext()` to access the shared state. The legacy Svelte 4 context has been removed.
...
- `onPointerMove(e, props, opts)` - Method for layers to call on pointer move
- `onPointerDown(e, props, opts)` - Method for layers to call on pointer down
- `onPointerOut(e)` - Method for layers to call on pointer out

### External custom layer example

To create a custom layer outside this component library:
...
All built-in layers (`SVGPolygonLayer`, `SVGPointLayer`, `SVGLabelLayer`) now use this rune-based context internally.
