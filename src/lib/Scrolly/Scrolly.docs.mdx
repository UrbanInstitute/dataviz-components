import * as Stories from "./Scrolly.stories.svelte"
import { Meta, Controls, ArgTypes, Markdown, Story } from "@storybook/blocks"

<Meta component={Stories.Scrolly} of={Stories} />

# Scrolly

A component that can be used to create "scrolly" sections and pages. This component is a wrapper around the [svelte-scroller](https://github.com/sveltejs/svelte-scroller) library. If you're project has needs that go beyond what this component can support, you should consider using `svelte-scroller` directly. But for many common scenarious, the options provided here should have you covered.

Basic usage:
```html
<script>
  import { Scrolly } from "@urban-institute/dataviz-components";

  const slides = ["Slide 1", "Slide 2", "Slide 3"];

</script>
<Scrolly slides={slides}>
  <div
    slot="background"
    style="width: 100%; height: 100vh; color: var(--color-white); font-weight: var(--font-weight-bold); background: var(--color-blue); display: flex; align-items: center; justify-content: center;"
  >
    Scrolly background
  </div>
</Scrolly>
````

<Story of={Stories.Default} />
<Controls />

## Background components

While this component provides much of what you need to get started with a scrolly segment, you'll need to create your own backgound component. This can range from a very simple component that displays and image or a chart, to something highly complex with bespoke animations and transitions. The sky is the limit and how you build this is up to you and the needs of your project, but there are a few core pieces of functionality this component provides that will help you out. The `scrolly` component provides key information about the current state of the scrolly via [Svelte's context api](https://svelte.dev/docs/svelte#setcontext) with a key of `scrolly` as stores, so you don't need to worry about bindings or passing the correct props to your slotted `background` component. The following stores can be accessed from inside your background component:

<Markdown>
{`| store  | Description                                                                                   |
| ----------- | --------------------------------------------------------------------------------------------- |
| index       | The index of the current slide. This variable can be used to determine what content should be displayed in the \`background\`|
| offset      | how far the section has scrolled past the threshold, as a value between 0 and 1 |
| progress |   how far the foreground has travelled, where 0 is the top of the foreground crossing \`top\`, and 1 is the bottom crossing \`bottom\`|
| slideHeight | The height in pixels of the current slide based on browser conditions. Can be used for sizing content in the background.|
| slideWidth | The width in pixels of the current slide based on browser conditions. Can be used for sizing content in the background.|
`}
</Markdown>

Since these variables are stores, they can be used in your background component like so:

```JavaScript
// access stores from scrolly context
const { index, offset, progress, slideHeight, slideWidth } = getContext("scrolly");

// use stores in your component
console.log("The current index is", $index);

```

## Custom foregrounds

Information about overriding the default foreground elements TK.
