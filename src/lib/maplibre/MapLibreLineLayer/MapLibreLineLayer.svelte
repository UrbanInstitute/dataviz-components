...
<script>
   ...
   * @property {string} [stroke=urbanColors.blue] Line color.
   * @property {number} [strokeWidth=1] Line width.
   * @property {number} [strokeOpacity=1] Line opacity.
   * @property {string} [hoverStroke] Line color on hover (via feature-state).
   */
  ...
  const layerId = id || generateId("line-layer");
  ...
  const computedPaint = $derived.by(() => {
    return {
      "line-color": hoverStroke
        ? ["case", ["boolean", ["feature-state", "hover"], false], hoverStroke, stroke]
        : stroke,
