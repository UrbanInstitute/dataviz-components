...
<script>
   ...
   * @property {string} [fill=urbanColors.blue] Fill color.
   * @property {string} [hoverFill] Fill color on hover (via feature-state).
   * @property {number} [fillOpacity=1] Fill opacity.
   * @property {string} [stroke=urbanColors.white] Outline color.
   */
  ...
  const layerId = id || generateId("fill-layer");
  ...
  const computedPaint = $derived.by(() => {
    ...
    if (stroke) {
      p["fill-outline-color"] = stroke;
    }
    if (hoverFill) {
      p["fill-color"] = ["case", ["boolean", ["feature-state", "hover"], false], hoverFill, fill];
    }
    return p;
  });
